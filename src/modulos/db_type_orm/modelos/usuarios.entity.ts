...
export class Usuarios{
    ...
    @Index()
    @ManyToOne((type)=>Estado, estado =>estado.id, {cascade: true, eager: false, nullable: false})
    @JoinColumn({name: 'estado_id'})
    estado_id: Estado
     
    @Index()
    @ManyToOne((type)=>Perfiles, perfiles =>perfiles.id, {cascade: true, eager: false, nullable: false})
